...
cleaned_NYPD_2013_2018 <- NYPD_Arrests_Data_Historic_ %>%
  mutate(ARREST_DATE = lubridate::mdy(ARREST_DATE)) %>%
  filter(ARREST_DATE > "2012-12-31" & ARREST_DATE < "2019-01-01") %>%
  group_by(ARREST_DATE,ARREST_PRECINCT) %>%
  tally() %>%
  rename(Count = n)
...
cleaned_NYPD_Arrests <- rbind(cleaned_NYPD_2019,cleaned_NYPD_2013_2018)
